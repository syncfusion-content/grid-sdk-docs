...
{% endtabs %}
> **Note:** The `dotnet new webapi` template includes a `WeatherForecast` sample by default. You can delete the `WeatherForecast.cs` model and `WeatherForecastController.cs` controller files generated by the template before continuing.
{% endtabcontent %}
...
> **Important:** `crudUrl` is mutually exclusive with the individual `insertUrl`, `updateUrl`, and `removeUrl` properties. When `crudUrl` is set, the DataManager ignores the other three and dispatches every CRUD action to the single endpoint, where the server-side handler branches on `request.action` (`"insert"`, `"update"`, or `"remove"`).
