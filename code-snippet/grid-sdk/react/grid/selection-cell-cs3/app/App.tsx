...
function App() {
  ...
  const showSelectedRecord = () => {
    selectedCellIndexes = (grid as GridComponent).getSelectedRowCellIndexes();
    setDialogVisible(true);
    (dialog as DialogComponent).content = `<p><b>Selected Cell Indexes:</b></p><br/>`
    for (let i = 0; i < selectedCellIndexes.length; i++) {
      (dialog as DialogComponent).content += `<p><li>Row: ${selectedCellIndexes[i].rowIndex}, cellIndex: ${selectedCellIndexes[i].cellIndexes} <p>`;
    }
  }
  const dialogClose = () => {
