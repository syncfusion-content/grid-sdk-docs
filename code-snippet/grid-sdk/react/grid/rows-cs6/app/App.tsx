...
function App() {
    ...
    const onCheckBoxChange = (args) => {
        if (args.checked) {
            for (let i = 0; i < (grid as GridComponent).getRowsObject().length; i++) {
                if ((grid as GridComponent).getRowsObject()[i].data.CustomerID === 'VINET') {
                    // check the row value
                    rowIndex = (grid as GridComponent).getRowsObject()[i].index; //get particular row index
                    (grid as GridComponent).getRowByIndex(rowIndex).style.display = 'none'; //hide row
