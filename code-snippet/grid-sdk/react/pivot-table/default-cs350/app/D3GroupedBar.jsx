...
function D3GroupedBar({ data, title }) {
  ...
  React.useEffect(() => {
    ...
    const rowKeys = unique(data.map((item) => item.row));
    const columnKeys = unique(data.map((item) => item.column));
    const legendColumns = Math.max(
      1,
      Math.floor((CHART_WIDTH - 100) / LEGEND_ITEM_WIDTH)
    );
    const legendRows = Math.ceil(columnKeys.length / legendColumns);
    const margin = {
    ...
    const x1 = d3.scaleBand()
      ...
      .padding(0.08);
    const maximumValue = d3.max(data, (item) => item.value) || 0;
    ...
    const legendItems = legend.selectAll('g')
      ...
      .attr('transform', (column, index) => {
        const x = (index % legendColumns) * LEGEND_ITEM_WIDTH;
        const yPosition = Math.floor(index / legendColumns) * 24;
    ...
    const groups = plot.selectAll('.bar-group')
      ...
      .attr('transform', (row) => `translate(${x0(row) || 0}, 0)`);

    const bars = groups.selectAll('rect')
      .data((row) => columnKeys.map((column) => ({
        row: row,
        column: column,
