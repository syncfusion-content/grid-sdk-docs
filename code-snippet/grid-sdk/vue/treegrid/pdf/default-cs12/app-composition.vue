...
import { TreeGridComponent, ColumnDirective, ColumnsDirective, AggregateDirective, AggregatesDirective, Aggregate, Page, Toolbar, PdfExport } from "@syncfusion/ej2-vue-treegrid";
...
const toolbarOptions = ref(["PdfExport"]);
...
// Initialize DropDownList in the footer
let listObj = null;
const dataBound = () => {
...
// PDF aggregate query cell info handler
const pdfAggregateQueryCellInfo = (args) => {
  if (args.cell.column.headerText === "Category") {
    args.value = `Count of ${selectedCategory.value}: ${args.row.data.category.Custom}`;
  }
};

// Provide dependencies
provide("treegrid", [Aggregate, Page, Toolbar, PdfExport]);
</script>
```