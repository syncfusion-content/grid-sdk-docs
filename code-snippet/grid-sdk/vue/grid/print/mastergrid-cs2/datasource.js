...
export var customerData = [
 ...
 export let orderData = JSON.parse(stringData, function (field, value) {
   if (typeof value === "string" && /^(\d{4}-\d\d-\d\d([tT][\d:.]*)?)([zZ]|([+-]\d\d:?(\d\d)?))?$/.test(value)) {
 ...
 // Mapping order data with customer names
 export let data = orderData.map(function (item) {
   const name = customerData.find(cItem => cItem.CustomerID === item.CustomerID);
